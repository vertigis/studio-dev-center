...
As of our 5.30 release, VertiGIS Studio Mobile has been migrated from Xamarin Forms to .NET Maui. We've now completed work to support Maui in our nuget package as well, so the time has come for any projects using our SDK to do their own migration.
...
There are some existing guides. For example Microsoft has written [_Upgrade a Xamarin.Forms app to a .NET MAUI app with the .NET Upgrade Assistant_](https://learn.microsoft.com/en-us/dotnet/maui/migration/upgrade-assistant?view=net-maui-9.0&tabs=vswin) and
...
The method I'll be going through will result in one shared project, and separate projects for each platform - the structure you're probably familiar with from your Xamarin Forms app.
In order to merge your projects into one and take advantage of Maui's convenient single-project structure, I recommend creating a brand new single-project Maui app and moving your files over, the way Microsoft describes in their "_Manually upgrade \[...]_" article I linked above.
This guide will not go through that step, but you are free to do that yourself at the end.
This guide will also not talk about the process of converting custom Renderers to use the new Maui Handler pattern, but if you have custom Renderers that is something you'll have to evaluate.
...
Once you've installed it, right-click each project and run the tool.
\[img]

Select "Upgrade project to a newer .NET version". Mobile currently supports .Net 8, so pick that as the version.

## Step 2 - Upgrade packages

The Upgrade Assistant will have done some package updates automatically, but not all of them.
If you're referencing any Esri ArcGIS Runtime packages, you'll need to upgrade those from 100.x to the 200.x version (as of writing this post, Mobile supports 200.5).
...
Important: [TODO: style this as a note somehow]
There's also a new step you'll need to add to use our package: in the MauiProgram.cs file for each of the platform projects, you'll need to add `UseStudioMobile` onto the builder. It should end up looking like this:
...
Do a search through your codebase for any remaining references to Xamarin.Forms. There are probably some instances that the Upgrade Assistant didn't do for you. For example:
...
(note the switch from UWP to WinUI).
...
[img of the incorrect configuration mappings]
...
This build error on Windows: `Type 'App' already defines a member called 'InitializeComponent' with the same parameter types`, is solved by adding the following lines to the csproj:
...
On Windows, if you were merging in Mobile's VertiGISResources file in a merged dictionary, like `<ResourceDictionary Source="VertiGIS.Mobile.UWP/Resources/VertiGISResources.xaml"/>`, you'll now need to update the path: `<ResourceDictionary Source="VertiGIS.Mobile.UWP/Resources/VertiGISResources.xaml"/>`.
`VertiGIS.Mobile.Toolkit.UWP/Resources/ToolkitResources.xaml` no longer exists.
