...
## 4 - Updating the Mobile app to override the Search functionality

To use this Workflow we've created in Mobile, open your app in Designer. Find your Search component, and scroll down to the Events portion of the configuration. Click the edit icon to modify the `Search` event's behaviour.

<img src={useBaseUrl("img/mobile-edit-search-button.png")} />

In the dialog which opens, select the Workflow you just created. If your Workflow does not need to display any UI (e.g. no Workflow Form involved), set the Target to `None` to indicate that it does not need to open a panel.

<img src={useBaseUrl("img/mobile-search-workflow-target-none.png")} />

Save the app. Now you can launch the app in Go and attempt to search for a feature that doesn't exist in the visible extent. If you've done everything correctly, the search should go through with the configured extent.
