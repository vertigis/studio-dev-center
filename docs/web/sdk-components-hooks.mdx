...
VertiGIS Studio Web provides a number of [React hooks](https://reactjs.org/docs/hooks-intro.html) to ease development of custom components bound to [models](sdk-components-reference.mdx#models). These hooks make it simple to follow the recommended [observable properties pattern](sdk-observability.mdx) for model interactions within your functional components.
...
Watches a property of an object that is [observable](sdk-observability.mdx) for changes and triggers a callback.

## `useWatchInit`

Watches a property of an object that is [observable](sdk-observability.mdx) for changes and triggers a callback. The callback will also be invoked immediately on first render.
