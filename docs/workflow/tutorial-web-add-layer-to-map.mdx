...
1. Add imports for the `MapProvider`, `activate`, and `IActivityContext` at the top of the `AddLayerToMap.ts` file.
1. Add the `@activate(MapProvider)` decorator to the `execute` method.
1. Modify the activity `execute` method to mark it as `async` (and return a `Promise`).
1. Modify the activity `execute` method to include the `context` and `type` parameters.
...
Next, we can use the `MapProvider` to get the map.
...
Next, we can use the [ArcGIS API for JavaScript](sdk-web-integrating-the-arcgis-api.mdx) to create a graphics layer and add it to the map.
